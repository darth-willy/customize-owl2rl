...
function defaultSelect(selections, config, callback) {
	for (var i = 0; i < selections.length; i++)
		selections[i] = loadSelection(selections[i]);
...
function domainBasedSelect(type, config, callback) {
	config.type = type;

	contactWebService(JSON.stringify(config), 'select/domain', [], callback);
}

function preprocess(type, config, callback) {
