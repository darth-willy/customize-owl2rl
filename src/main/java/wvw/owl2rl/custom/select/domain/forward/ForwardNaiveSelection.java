...
public class ForwardNaiveSelection extends DomainBasedSelection {
	...
	public List<IRule> selectRuleset(IRuleset ruleset, OntologyConfig ontology, DomainBasedSelectConfig config)
		...
		while (true) {
			...
			while (ruleIt.hasNext()) {
				...
				QueryExecution exec = QueryExecutionFactory.create(rule.getQuery(), model);
			...
			if (model.size() == size) {
				...
				System.out.println("\ndone (# rules: (new) " + nrRules1 + " <> (old) " + nrRules0 + ": " + diff + ")");
