...
import wvw.owl2rl.custom.select.domain.rule.struct.tp.Lit;
import wvw.owl2rl.custom.select.domain.rule.struct.tp.Res;
import wvw.owl2rl.custom.select.domain.rule.struct.tp.Term;
import wvw.owl2rl.custom.select.domain.rule.struct.tp.TriplePat;
import wvw.owl2rl.custom.select.domain.rule.struct.tp.Val;
import wvw.owl2rl.custom.select.domain.rule.struct.tp.Var;
import wvw.utils.rdf.OwlNS;
import wvw.utils.rdf.RdfNS;
import wvw.utils.rdf.RdfsNS;
...
public class RDFUtils {
	...
	public static boolean isTypePred(RDFNode node) {
		return (RdfNS.URI + "type").equals(node.toString());
	}

	public static boolean isCustomPred(RDFNode node) {
		return !node.toString().startsWith(RdfNS.URI)
				&& !node.toString().startsWith(RdfsNS.URI)
				&& !node.toString().startsWith(OwlNS.URI);
	}
}
